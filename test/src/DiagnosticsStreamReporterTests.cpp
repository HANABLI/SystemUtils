...
struct DiagnosticsStreamReporterTests : public ::testing::Test
{
    ...
    virtual void TearDown() { ASSERT_TRUE(SystemUtils::File::DeleteDirectory(testAreaPath)); }
};
...
TEST_F(DiagnosticsStreamReporterTests, SaveDiagnosticMessagesToLogFiles) {
    ...
    const auto unsubscribeDelegate =
        sender.SubscribeToDiagnostics(SystemUtils::DiagnosticsStreamReporter(output, error));
