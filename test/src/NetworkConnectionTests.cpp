...
TEST_F(NetworkConnectionTests, NetworkConnectionTests_EstablishConnection__Test) {
    ...
    const auto newConnectionDelegate =
        [&clients, &callbackCondition,
         &callbackMutex](std::shared_ptr<SystemUtils::NetworkConnection> newConnection)
    {
    ...
    const auto packetReceiveDelegate = [](
                                           ...
                                           const std::vector<uint8_t>& body) {};
    ASSERT_TRUE(server.Open(newConnectionDelegate, packetReceiveDelegate,
    ...
    ASSERT_TRUE(client.Connect(0x7F000001, server.GetBoundPort()));
    {
        ...
        ASSERT_TRUE(callbackCondition.wait_for(lock, std::chrono::seconds(1),
...
TEST_F(NetworkConnectionTests, NetworkConnectionTests_SendingMessage_Test) {
    ...
    std::mutex callbackMutex;
    const auto newConnectionDelegate =
        [&clients, &callbackCondition, &callbackMutex,
         &serverConnectionOwner](std::shared_ptr<SystemUtils::NetworkConnection> newConnection)
    {
        std::unique_lock<std::mutex> lock(callbackMutex);
        clients.push_back(newConnection);
...
TEST_F(NetworkConnectionTests, NetworkConnectionTests_ReceivingMessage_Test) {
    ...
    const auto packetReceiveDelegate = [](uint32_t address, uint16_t port,
                                          const std::vector<uint8_t>& body) {};
    ASSERT_TRUE(server.Open(newConnectionDelegate, packetReceiveDelegate,
                            SystemUtils::NetworkEndPoint::Mode::Connection, 0, 0, 0));
    ASSERT_TRUE(client.Connect(0x7F000001, server.GetBoundPort()));
