...
int main(int argc, char* argv[]) {
    const std::string pidFilePath = SystemUtils::File::GetExeParentDirectory() + "/TestArea/pid";
    auto pidFile = fopen(pidFilePath.c_str(), "w");
    (void)fprintf(pidFile, "%u", SystemUtils::Subprocess::GetCurrentProcessId());
    (void)fclose(pidFile);
    SystemUtils::Subprocess parent;
    ...
    if ((args.size() >= 1) && (args[0] == "child"))
    {
        ...
        { return EXIT_FAILURE; }
    } else if ((args.size() >= 1) && (args[0] == "detached"))
    {
#ifdef _WIN32
#elif defined(__APPLE__)
        ...
        { return EXIT_FAILURE; }
        std::ofstream report(
            (SystemUtils::File::GetExeParentDirectory() + "/TestArea/handles").c_str());
        ...
        SystemUtils::File handlesReport(SystemUtils::File::GetExeParentDirectory() +
                                        "/TestArea/handles");
    ...
    const std::string testFilePath =
        SystemUtils::File::GetExeParentDirectory() + "/TestArea/foo.txt";
    auto f = fopen(testFilePath.c_str(), "w");
    for (const auto& arg : args)
    { (void)fprintf(f, "%s\n", arg.c_str()); }
    (void)fclose(f);
    if ((args.size() >= 2) && (args[1] == "crash"))
    {
        volatile int* null = (int*)0;
        *null = 0;
    } else if ((args.size() >= 2) && (args[1] == "handles"))
    {
#ifdef _WIN32
#elif defined(__APPLE__)
        auto bufferSize = proc_pidinfo(getpid(), PROC_PIDLISTFDS, 0, 0, 0);
        ...
        for (const auto& fd : fds)
        {
            if ((fd != pipeToParentFdPath) && (readlink(fd.c_str(), link.data(), link.size()) >= 0))
            { report << fd << std::endl; }
        }
        SystemUtils::File handlesReport(SystemUtils::File::GetExeParentDirectory() +
                                        "/TestArea/handles");
