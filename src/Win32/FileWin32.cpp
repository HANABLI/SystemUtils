...
namespace SystemUtils
{
    ...
    bool File::OpenReadOnly() {
        ...
        impl_->platform_->handle =
            CreateFileA(impl_->path.c_str(), GENERIC_READ,
    ...
    bool File::OpenReadWrite() {
        ...
        {
            impl_->platform_->handle =
                CreateFileA(impl_->path.c_str(), GENERIC_READ | GENERIC_WRITE,
                            FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                            OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
            if (impl_->platform_->handle == INVALID_HANDLE_VALUE)
            {
                if (createPathTried || !impl_->CreatePath(impl_->path))
                { return false; }
    ...
    bool File::CopyDirectory(const std::string& existingDirectory,
        ...
        if ((newDirectoryWithSeparator.length() > 0) &&
    ...
    std::vector<std::string> File::GetDirectoryRoots() {
        ...
        std::shared_ptr<char> driveStringsBuffer(new char[driveStringsBufferSize],
                                                 [](char* p) { delete[] p; });
        if (GetLogicalDriveStringsA((DWORD)driveStringsBufferSize, driveStringsBuffer.get()) > 0)
        {
            ...
            for (;;)
            {
                ...
                { break; }
                size_t k = j;
                while ((k > i) && ((driveStringsBuffer.get()[k - 1] == '/') ||
                                   (driveStringsBuffer.get()[k - 1] == '\\')))
                { --k; }
                roots.emplace_back(driveStringsBuffer.get() + i, driveStringsBuffer.get() + k);
                i = j + i;
            }
            return roots;
    ...
    uint64_t File::GetPosition() const {
        ...
        if (SetFilePointerEx(impl_->platform_->handle, distanceToMove, &newPosition,
                             FILE_CURRENT) == 0)
        { return 0; }
        ...
        if (impl_->platform_->handle != INVALID_HANDLE_VALUE)
        {
            if (clone->impl_->platform_->writeAccess)
            {
                clone->impl_->platform_->handle =
                    CreateFileA(clone->impl_->path.c_str(), GENERIC_READ | GENERIC_WRITE,
                                FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                                OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
            } else
            {
                clone->impl_->platform_->handle =
                    CreateFileA(clone->impl_->path.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL,
