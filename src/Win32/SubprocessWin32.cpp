 ...
 *
 * This module contains the Windows impementation of the
 * SystemUtils::Subprocess class.
 *
 * © 2024 by Hatem Nabli
 */
...
namespace
{
     ...
     */
    std::vector<char> MakeCommandLine(const std::string& program,
...
{
    /**
     * This contains the private methods and properties of
    ...
    struct Subprocess::Impl
    {
         ...
         * is used un the parent role. The thread basically waits
         * for one of two things to happen:
         * - The child process writes to the pipe, sinaling that it's
        ...
        /**
         * This is a callback to call when the child process
        ...
        /**
         * This si the operating system handle to the child process object.
         * It's used to block until the child process is completely cleaned
         * up (after the child process exits or crashees).
         */
        HANDLE child = INVALID_HANDLE_VALUE;

        /**
         * This is the pipe used to communicate between the
         * parent and child process. The parent only reads it, to know
         ...
         * which is temporarilly ignored while minitoring the child process.
         */
        void (*previousSignalHandler)(int) = nullptr;

        /**
         * This is th temporary handler for the SIGINT signal,
         ...
         * it does nothing.
         * When the parent process is exiting, in which case it joins
         * the monitor/worker thread, in order to ensure the child process
         * exits first.
    ...
    Subprocess::Subprocess() : impl_(new Impl()) {}
    ...
    unsigned int Subprocess::StartChild(std::string program, const std::vector<std::string>& args,
        ...
        if ((program.length() < 4) || (program.substr(program.length() - 4) != ".exe"))
        { program += ".exe"; }

        std::vector<std::string> childArgs;
        childArgs.push_back("child");
        childArgs.push_back(StringUtils::sprintf("%" PRIu64, (uint64_t)childPipe));
        ...
        if (CreateProcessA(program.c_str(), &commandLine[0], NULL, NULL, TRUE, 0, NULL, NULL, &si,
                           &pi) == 0)
        {
            (void)CloseHandle(impl_->pipe);
    ...
    unsigned int Subprocess::StartDetached(std::string program,
        ...
        si.cb = sizeof(si);
        PROCESS_INFORMATION pi;
        if (CreateProcessA(program.c_str(), &commandLine[0], NULL, NULL, FALSE, DETACHED_PROCESS,
                           NULL, NULL, &si, &pi) == 0)
        { return 0; }
    ...
    unsigned int Subprocess::GetCurrentProcessId() { return (unsigned int)::GetCurrentProcessId(); }
    ...
    auto Subprocess::GetProcessList() -> std::vector<ProcessInfo> {
        ...
        {
            DWORD bytesNeeded;
            if (EnumProcesses(processIds.data(), (DWORD)(processIds.size() * sizeof(DWORD)),
                              &bytesNeeded) == FALSE)
            { return {}; }
        ...
        do
        {
            ...
            getTcpTableResult =
                GetTcpTable2((PMIB_TCPTABLE2)tcpTableBuffer.data(), &requiredTcpTableSize, FALSE);
        ...
        // For each proocess ID, attempt to access the process to get its image
        // executable path.
        // If successful, add the process information to the returned collection.
        ...
        for (size_t i = 0; i < processIds.size(); ++i)
        {
            ...
            if (EnumProcessModules(processHandle, &processModule, sizeof(processModule),
                                   &processModuleSize) == FALSE)
            {
                (void)CloseHandle(processHandle);
                continue;
            }
            std::vector<char> exeImagePath(MAX_PATH + 1);
            (void)GetModuleFileNameExA(processHandle, processModule, &exeImagePath[0],
