...
namespace
{
    ...
    struct Subscription
    {
        ...
        /**
         * This a function to call to deliver messages
         * to this subscriber.
         */
        ...
        /**
         * This constructor is used to initialize all properties
         * of an instance.
         *
         ...
         *       This is the minimum level of message that this
         *       subscriber desires to receive.
         */
        Subscription(SystemUtils::DiagnosticsSender::DiagnosticMessageDelegate newDelegate,
                     size_t newMinLevel) :
            delegate(newDelegate), minLevel(newMinLevel) {}
    };
}  // namespace
namespace SystemUtils
{
    struct DiagnosticsSender::Impl
    {
        ...
        /**
         * This is the minimum desired message
         * levels for all current subscribers.
         */
        size_t minLevel = std::numeric_limits<size_t>::max();

        /**
         * This is used to include extra contextual information in front
         * of any messages published from this sender.
         */
    ...
    DiagnosticsSender::DiagnosticsSender(std::string name) : impl_(std::make_shared<Impl>()) {
        impl_->name = name;
    }

    auto DiagnosticsSender::SubscribeToDiagnostics(DiagnosticMessageDelegate delegate,
                                                   size_t minLevel) -> UnsubscribeDelegate {
    ...
    auto DiagnosticsSender::Chain() const -> DiagnosticMessageDelegate {
        std::weak_ptr<Impl> implWeak(impl_);
        return [implWeak](std::string senderName, size_t level, std::string message)
        {
            const auto impl = implWeak.lock();
            if (impl == nullptr)
            { return; }
            impl->SendDiagnosticInformationString(level, senderName + ": " + message);
        };
    }

    size_t DiagnosticsSender::GetMinLevel() const { return impl_->minLevel; }

    void DiagnosticsSender::SendDiagnosticInformationString(size_t level,
                                                            std::string message) const {
        impl_->SendDiagnosticInformationString(level, message);
    }

    void DiagnosticsSender::SendDiagnosticInformationFormatted(size_t level, const char* format,
                                                               ...) const {
        if (level < impl_->minLevel)
        { return; }
        va_list args;
        va_start(args, format);
        const std::string message = StringUtils::vsprintf(format, args);
        va_end(args);
        impl_->SendDiagnosticInformationString(level, message);
    }
