...
namespace SystemUtils
{
    struct StringFile::Impl
    {
        ...
        /**
         * This is the current position of the file.
         */
        size_t position = 0;
    };

    StringFile::StringFile(std::string initialValue) : impl_(new Impl()) {
        impl_->value.assign(initialValue.begin(), initialValue.end());
    }
    StringFile::StringFile(std::vector<uint8_t> initialValue) : impl_(new Impl()) {
        impl_->value.assign(initialValue.begin(), initialValue.end());
    }
    StringFile::~StringFile() noexcept = default;
    StringFile::StringFile(const StringFile& other) : impl_(new Impl()) { *impl_ = *other.impl_; }
    ...
    void StringFile::Remove(size_t numBytes) {
        impl_->value.erase(impl_->value.begin(),
                           impl_->value.begin() + std::min(numBytes, impl_->value.size()));
        impl_->position = std::max(numBytes, impl_->position) - numBytes;
    }

    uint64_t StringFile::GetSize() const { return (uint16_t)impl_->value.size(); }
    ...
    uint64_t StringFile::GetPosition() const { return (uint64_t)impl_->position; }

    void StringFile::SetPosition(uint64_t position) { impl_->position = (size_t)position; }
    ...
    size_t StringFile::Peek(void* buffer, size_t numBytes) const {
        const size_t amountCopied = std::min(
            numBytes, impl_->value.size() - std::min(impl_->value.size(), impl_->position));
